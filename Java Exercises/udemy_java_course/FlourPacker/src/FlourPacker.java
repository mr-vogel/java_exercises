 ...
 */
    ...
    public static boolean canPack(int bigCount, int smallCount, int goal) {
        ...
        if ((bCWeight * bigCount) + (sCWeight * smallCount) < goal) {
            return false;
        } else if (((bCWeight * bigCount) + (sCWeight * smallCount)) == goal) {
            return true;
        }

        while ((bigCount > 0) && (currentWeight + 5 <= goal)){
            currentWeight += bCWeight;
            bigCount--;
        ...
        while((smallCount > 0) && (currentWeight + 1 <= goal)){
            currentWeight += sCWeight;
            smallCount--;
            if (currentWeight == goal) {
                return true;
            }
...
For example, if goal = 9, bigCount = 2, and smallCount = 0, t
he method should return false since each big bag is 5 kilos and cannot be divided.
